...
public class GlobalCacheBuilder {
    ...
    final static Logger logger = LoggerFactory.getLogger(GlobalCacheBuilder.class);
    ...
    private double memoryRatioForCache = defaultPolicyRatio; // if set, the cache will occupy this ratio of the max memory
    ...
    protected GlobalCacheBuilder() {
        if (Runtime.getRuntime().maxMemory() == Long.MAX_VALUE) {
            ...
            maxAvailableMemoryInBytes = (long) (Runtime.getRuntime().maxMemory() * 0.5);
        }
    }
    ...
    public AbstractGlobalCache create() {
        ...
        if (memoryRatioForCache>0) {
            ...
            if (memoryInBytesForCache> maxAvailableMemoryInBytes) {
                logger.warn("You are setting a cache size ("+memoryInBytesForCache+")which is higher than the total available memory from the JVM ("+ maxAvailableMemoryInBytes +")!");
            }
        ...
        logger.info("Global cache set to "+(int)(cacheSize/(1024.0*1024.0))+" Mb, out of "
                +(int)(maxAvailableMemoryInBytes/(1024.0*1024.0))
                +" Mb available ("+(int)(100.0*(cacheSize/(1024.0*1024.0))/(maxAvailableMemoryInBytes/(1024.0*1024.0)))+"%)");

        switch (cacheType) {
            case CAFFEINE: return new CaffeineGlobalCache(cacheSize, log, msBetweenLog);
            case LINKED_HASH_MAP: return new BoundedLinkedHashMapGlobalCache(100, cacheSize, log, msBetweenLog);
